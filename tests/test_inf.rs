...
mod inf_integration_tests {
    ...
    fn test_base_lib() {
        logger_init();
        let mut infp = InfParser::new(None);
        infp.parse_file(PathBuf::from("tests/data/baseLib.inf")).unwrap();
    ...
    fn parse_openssl_lib() {
        ...
        infp.parse_file(PathBuf::from("tests/data/opensslLib.inf")).unwrap();
